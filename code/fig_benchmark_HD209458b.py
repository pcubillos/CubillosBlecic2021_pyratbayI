...
pnames2 = mcmc_p2019['texnames'][6:]
pnames2 = [
    pname.replace('log_{10}','log').replace('planet','p').replace('f_','').
        replace('Ray','ray')
    for pname in pnames2]
pnames2[9] = '$\\log(f_{\\rm ray})$'
ranges2 = [
...
axes = [mc3.plots.subplotter(rect5, margin2, j+1,
            nx=nx, ny=ny, ymargin=ymargin) for j in range(nhist)]
idx = np.array([i for i in range(nhist) if i not in [13,14,18]])
zaxes = mc3.plots.histogram(
    mm2017_posterior,
    pnames=mcmc_mm2017['texnames'], ranges=np.array(ranges1+ranges2)[idx],
    quantile=0.683,
    axes=np.array(axes)[idx], fs=fs-2.0, lw=1.2, yscale=None, theme='blue')

axes = mc3.plots.histogram(
    p2019_posterior,
    pnames=mcmc_p2019['texnames'], ranges=ranges1+ranges2, quantile=0.683,
...
axes[0].legend([(handles[0],handles[2]),
